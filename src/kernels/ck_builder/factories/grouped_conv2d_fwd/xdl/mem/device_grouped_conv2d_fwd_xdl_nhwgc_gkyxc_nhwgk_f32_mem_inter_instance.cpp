    ...
    std::vector<std::unique_ptr<DeviceOp>>& instances)
{
    ...
    add_device_operation_instances<device_grouped_conv_fwd_xdl_f32_mem_instances<0>(
        2, NHWGC, GKYXC, {}, NHWGK, ConvFwdDefault, Interwave)>(instances);

    add_device_operation_instances<device_grouped_conv_fwd_xdl_f32_mem_instances<0>(
        2, NHWGC, GKYXC, {}, NHWGK, ConvFwd1x1P0, Interwave)>(instances);

    add_device_operation_instances<device_grouped_conv_fwd_xdl_f32_mem_instances<0>(
        2, NHWGC, GKYXC, {}, NHWGK, ConvFwd1x1S1P0, Interwave)>(instances);

    add_device_operation_instances<device_grouped_conv_fwd_xdl_f32_mem_instances<0>(
        2, NHWGC, GKYXC, {}, NHWGK, ConvFwdOddC, Interwave)>(instances);
}
