...
#include <miopen/ck_builder/factories/grouped_conv_fwd/device_grouped_conv_fwd_wmma_cshufflev3_wave_transfer_instance.hpp>
    ...
    std::vector<std::unique_ptr<DeviceOp>>& instances)
{
    ...
    constexpr auto GemmDefault    = ckb::GemmSpecialization::Default;

    // Data type aliases
    constexpr auto BF16 = ckb::DataType::BF16;

    add_device_operation_instances<device_grouped_conv_fwd_wmma_cshufflev3_wave_transfer_instances<
        0>(2, NHWGC, GKYXC, {}, NHWGK, ConvFwdDefault, GemmMNKPadding, BF16)>(instances);

    add_device_operation_instances<device_grouped_conv_fwd_wmma_cshufflev3_wave_transfer_instances<
        0>(2, NHWGC, GKYXC, {}, NHWGK, ConvFwd1x1S1P0, GemmDefault, BF16)>(instances);
}
