    ...
    std::vector<std::unique_ptr<DeviceOp>>& instances)
{
    ...
    add_device_operation_instances<device_grouped_conv2d_fwd_dl_f32_instances<0>(
        NHWGC, GKYXC, {}, NHWGK, {}, PassThrough, ConvFwdDefault)>(instances);

    add_device_operation_instances<device_grouped_conv2d_fwd_dl_f32_instances<0>(
        NHWGC, GKYXC, {}, NHWGK, {}, PassThrough, ConvFwd1x1P0)>(instances);

    add_device_operation_instances<device_grouped_conv2d_fwd_dl_f32_instances<0>(
        NHWGC, GKYXC, {}, NHWGK, {}, PassThrough, ConvFwd1x1S1P0)>(instances);
}
